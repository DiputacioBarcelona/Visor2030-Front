<template>
  <div>
    ...
    <IndicatorDistribution
      :data="beeswarmData"
      :selected-id="comarcaMunicipalityIds"
...
const { sdg, pieces, fullIndicatorId, targetId, indicatorId, comarcaId } =
  useFilters();
...
const mapFilter = computed(() => ({
  ...
  "municipality.comarca.comarca_code": data.value?.comarca_code,
}));
...
const comarcaCode = computed(() => {
  return data.value?.comarca_code?.padStart(2, "0");
});

const comarcaMunicipalityIds = computed(() =>
  municipalities
    .filter((m) => m.comarca.comarca_code === data.value?.comarca_code)
    .map((m) => m.id),
);
...
const {
  ...
  primaryEndpoint: "getComarcaValues",
  secondaryEndpoints: [
